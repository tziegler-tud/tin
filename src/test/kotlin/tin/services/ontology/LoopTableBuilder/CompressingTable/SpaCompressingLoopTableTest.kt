package tin.services.ontology.LoopTableBuilder.CompressingTable
...
import tin.services.ontology.loopTable.LoopTableBuilder.ELHI.ELHISPALoopTableBuilderCompressing
...
@SpringBootTest
@TestConfiguration
class SpaCompressingLoopTableTest {
    @Autowired
...
//        val exampleFile = readWithFileReaderService("univ-bench.owl.rdf").get()
    ...
    fun testLoopTableBasics(){
        ...
        val ec = manager.createELHINumericExecutionContext(false);
//        ec.prewarmSubsumptionCache()
        val defaultBuilder = ELHISPALoopTableBuilder(query.graph, transducer.graph, manager, ec);
        val compressingBuilder = ELHISPALoopTableBuilderCompressing(query.graph, transducer.graph, manager, ec);
//        val defaultTable = defaultBuilder.calculateFullTable();
        val compressingTable = compressingBuilder.calculateFullTable();

        val stats = defaultBuilder.getExecutionContext().dlReasoner.getStats();
