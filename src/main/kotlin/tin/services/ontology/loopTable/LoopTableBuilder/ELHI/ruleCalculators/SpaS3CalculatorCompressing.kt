...
class SpaS3CalculatorCompressing(
    ...
    fun calculateAll(M: NumericConceptNameRestriction, table: ELHISPALoopTableCompressing): Map<SPANumericLoopTableEntry, Int> {
        ...
        val updateMap: MutableMap<SPANumericLoopTableEntry, Int> = mutableMapOf();
        ...
        distance.forEach { (nodePair, distance) ->
            ...
            val entry = SPANumericLoopTableEntry(sourcePairNode.getQueryNode(), sourcePairNode.getTransducerNode(), targetPairNode.getQueryNode(), targetPairNode.getTransducerNode(), M);
    ...
    //use Floyd Warshall to calculate all possible updates in one step
    fun calculateAllV2(table: ELHISPALoopTableCompressing): Map<SPANumericLoopTableEntry, Int> {
        ...
        val graphMap: HashMap<NumericConceptNameRestriction, GenericGraph> = hashMapOf()
        ...
        graphMap.forEach { (restriction, graph) ->
            ...
            distance.forEach { (nodePair, distance) ->
                ...
                //build SPALoopTableEntry
                val entry = SPANumericLoopTableEntry(sourcePairNode.getQueryNode(), sourcePairNode.getTransducerNode(), targetPairNode.getQueryNode(), targetPairNode.getTransducerNode(), restriction);
