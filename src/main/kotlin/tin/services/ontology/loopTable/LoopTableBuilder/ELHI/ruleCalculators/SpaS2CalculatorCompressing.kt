...
class SpaS2CalculatorCompressing(
     ...
     */
    fun calculateAll(table: ELHISPALoopTableCompressing) : ELHISPALoopTableCompressing {
        val newTable: ELHISPALoopTableCompressing = ELHISPALoopTableCompressing(ec.spaRestrictionBuilder)
...
//                                if (tailset.containsElement(edgeClass)) {
                                if (tailset.isSupersetOf(edgeClassRestriction)) {
                                    newTable.setIfLower(entry, cost );
                                    return@tailsets
                                }
