...
import typeSpecifications.databaseSpecification.DatabaseEdge;
...
import typeSpecifications.querySpecification.QueryEdge;
import typeSpecifications.querySpecification.QueryGraph;
import typeSpecifications.querySpecification.QueryNode;
import typeSpecifications.transducerSpecification.TransducerEdge;
...
public class ProductAutomatonConstructor {
    ...
    public void construct() {
        ...
        //part (I)
        for (QueryNode queryNode : queryGraph.getNodes()) {
            for (QueryEdge queryEdge : queryNode.getEdges()) {
                String localQueryLabel = queryEdge.getLabel();
                ...
                for (TransducerNode transducerNode : transducerGraph.nodes) {
                    for (TransducerEdge transducerEdge : transducerNode.getEdges()) {
                        if (localQueryLabel.equals(transducerEdge.getIncomingString())) {
                            fittingTransducerEdges.add(transducerEdge);
                        }

                        // add incoming epsilon edges where they can be applied
                        if (queryNode.isFinalState()) {
                            if (transducerEdge.getIncomingString().isBlank()) {
                                fittingTransducerEdges.add(transducerEdge);
                            }
                        }
                    }
                ...
                for (DatabaseNode databaseNode : databaseGraph.nodes) {
                    for (DatabaseEdge databaseEdge : databaseNode.getEdges()) {
                        String localDatabaseLabel = databaseEdge.getLabel();
                        ...
                        for (TransducerEdge transducerEdge : fittingTransducerEdges) {
                            ...
                            if (transducerEdge.getIncomingString().isBlank()) {
                                ...
                                if (transducerEdge.getOutgoingString().isBlank()) {
                                    ...
                                    productAutomatonGraph.addProductAutomatonEdge(source, target, "", "", transducerEdge.getCost());


                                } else if ((isNegated(transducerEdge.getOutgoingString())) && (localDatabaseLabel.equals(unNegateString(transducerEdge.getOutgoingString())))) {
                                    ...
                                    source = getInstance(temporaryNodes, source); // duplicate check
                                    target = getInstance(temporaryNodes, target); // duplicate check
                                    productAutomatonGraph.addProductAutomatonEdge(source, target, "", transducerEdge.getOutgoingString(), transducerEdge.getCost());
                                } else if (!isNegated(transducerEdge.getOutgoingString()) && (localDatabaseLabel.equals(transducerEdge.getOutgoingString()))) {
                            ...
                            } else if (!isNegated(transducerEdge.getIncomingString())) {
                                ...
                                if (transducerEdge.getOutgoingString().isBlank()) {
                                    ...
                                    source = getInstance(temporaryNodes, source); // duplicate check
                                    target = getInstance(temporaryNodes, target); // duplicate check
                                    productAutomatonGraph.addProductAutomatonEdge(source, target, transducerEdge.getIncomingString(), "", transducerEdge.getCost());
                                } else if ((!isNegated(transducerEdge.getOutgoingString())) && (localDatabaseLabel.equals(transducerEdge.getOutgoingString()))) {
                                    ...
                                    source = getInstance(temporaryNodes, source); // duplicate check
                                    target = getInstance(temporaryNodes, target); // duplicate check
                                    productAutomatonGraph.addProductAutomatonEdge(source, target, transducerEdge.getIncomingString(), transducerEdge.getOutgoingString(), transducerEdge.getCost());
                                } else if ((isNegated(transducerEdge.getOutgoingString())) && (localDatabaseLabel.equals(unNegateString(transducerEdge.getOutgoingString())))) {
    ...
    private Pair<ProductAutomatonNode, ProductAutomatonNode> constructAutomatonNode(QueryEdge queryEdge, TransducerEdge transducerEdge, DatabaseEdge databaseEdge, EdgeType edgeType) {
        ...
        switch (edgeType) {
            ...
            case epsilonIncomingPositiveOutgoing:
                ...
                sourceInitialState = (queryEdge.getSource().isInitialState() && transducerEdge.getSource().isInitialState());
                sourceFinalState = (queryEdge.getSource().isFinalState() && transducerEdge.getSource().isFinalState());

                targetInitialState = (queryEdge.getSource().isInitialState() && transducerEdge.getTarget().isInitialState());
                targetFinalState = (queryEdge.getSource().isFinalState() && transducerEdge.getTarget().isFinalState());

                sourceNode = new ProductAutomatonNode(queryEdge.getSource(), transducerEdge.getSource(), databaseEdge.getSource(), sourceInitialState, sourceFinalState);
                targetNode = new ProductAutomatonNode(queryEdge.getSource(), transducerEdge.getTarget(), databaseEdge.getTarget(), targetInitialState, targetFinalState);
            ...
            case epsilonIncomingNegativeOutgoing:
                ...
                sourceNode = new ProductAutomatonNode(queryEdge.getSource(), transducerEdge.getSource(), databaseEdge.getTarget(), sourceInitialState, sourceFinalState);
                targetNode = new ProductAutomatonNode(queryEdge.getSource(), transducerEdge.getTarget(), databaseEdge.getSource(), targetInitialState, targetFinalState);
                break;
            ...
            case positiveIncomingPositiveOutgoing:
                ...
                targetInitialState = (queryEdge.getTarget().isInitialState() && transducerEdge.getTarget().isInitialState());
                targetFinalState = (queryEdge.getTarget().isFinalState() && transducerEdge.getTarget().isFinalState());

                sourceNode = new ProductAutomatonNode(queryEdge.getSource(), transducerEdge.getSource(), databaseEdge.getSource(), sourceInitialState, sourceFinalState);
                targetNode = new ProductAutomatonNode(queryEdge.getTarget(), transducerEdge.getTarget(), databaseEdge.getTarget(), targetInitialState, targetFinalState);
                break;
            case positiveIncomingNegativeOutgoing:
                ...
                sourceNode = new ProductAutomatonNode(queryEdge.getSource(), transducerEdge.getSource(), databaseEdge.getTarget(), sourceInitialState, sourceFinalState);
                targetNode = new ProductAutomatonNode(queryEdge.getTarget(), transducerEdge.getTarget(), databaseEdge.getSource(), targetInitialState, targetFinalState);
                break;
    ...
    private ProductAutomatonNode getInstance(HashMap<String, ProductAutomatonNode> nodeMap, ProductAutomatonNode node) {
        ...
        if (nodeMap.containsKey(node.getIdentifierString())) {
            node = nodeMap.get(node.getIdentifierString());

        } else nodeMap.put(node.getIdentifierString(), node);
